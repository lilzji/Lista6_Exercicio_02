...
public class ArquivosController implements IArquivosController {
	...
	public void readFile(String path, String nome, String ano, String mes, Double media) throws IOException {
		...
		if (arq.exists() && arq.isFile()) {
			...
			while (linha != null) {
				...
				if (dados[1].equals(ano) && dados[2].equals(mes) && Double.parseDouble(dados[3]) >= media) {
					System.out.println(dados[0] + " | " + dados[3]);
				}
	...
	public void createFile(String path, String nome, String[] conteudo) throws IOException {
		File dir = new File(path);
		File arq = new File(path, "teste.csv");
		if (dir.exists() && dir.isDirectory()) {
			boolean existe = false;
			if (arq.exists()) {
				existe = true;
			}
			String texto = geraCsv(conteudo);
	...
	private String geraCsv(String[] conteudo) {
		StringBuffer buffer = new StringBuffer();
		String linha = "";
		while (!linha.equalsIgnoreCase(null)) {
			linha = conteudo[1] + ";" + conteudo[3];
